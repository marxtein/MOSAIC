...
classdef ps_cls2
    methods(Static)
        %%
%(particle.charge,particle.mass,E0,E1,E_grid,Eperp_in,Pzeta0,Pzeta1,Pzeta_grid,E_max,E_grid_num,psi_diag_norm)
        function A =surf_Pzeta_E_mu(qpart,apart,E0_keV,E1_keV,E_grid,Pzeta0,Pzeta1,Pzeta_grid,E_max,E_grid_num,psi_diag_norm)
            ...
            for i = 1:A.E_grid
                ...
                if A.energy(7,i,1)==0
                    ...
                    MG = ps_cls2.Pzeta_E_fixed_mu(A.charge,A.mass,A.energy(7,i,1),Pzeta0,Pzeta1,Pzeta_grid,E_max,E_grid_num,psi_diag_norm);
    ...
    %GB = ps_cls2.Pzeta_lambda_fixed_mu(particle.charge,particle.mass,Eperp_in,Pzeta0,Pzeta1,Pzeta_grid_num,lam_min,lam_grid_num,sign_gpsi);
        ...
        function A = Pzeta_E_fixed_mu(qpart,apart,Eperp_in,Pzeta0,Pzeta1,Pzeta_grid_num,E_max,E_grid_num,psi_diag_norm)
            ...
            %A.E_pzeta(3,:) = (qpart^2/apart)*(A.pzeta_norm*eq.psiw).^2/(2*gpsi_axis^2)*baxis^2 + mu_norm*bmin;%wzk
            ...
            Atmp = ps_cls2.find_Pzeta_right_fixed_mu(qpart,apart,Eperp_in,Eperp_in/min(A.E_pzeta(1,:)),0.96*Eperp_in/max(A.E_uni_grid),E_grid_num,sign_gpsi);
            A.E_uni_grid1 = Eperp_in./Atmp.lam;
            % A.Pzeta_bound0 = interp1(lambda_tmp0, Pzeta_tmp0, A.lambda_uni_grid, 'spline');
            % A.Pzeta_bound0 = interp1(E_tmp1, Pzeta_tmp1, A.E_uni_grid1, 'spline');
            % A.Pzeta_bound1 = interp1(E_tmp2, Pzeta_tmp2, A.E_uni_grid1, 'spline');
            % A.Pzeta_bound2 = interp1(E_tmp32, Pzeta_tmp32, A.E_uni_grid1, 'spline');
            % A.Pzeta_bound3 = interp1(E_tmp4, Pzeta_tmp4, A.E_uni_grid1, 'spline');
            % A.Pzeta_bound2 = interp1(lambda_tmp12, Pzeta_tmp12, A.lambda_uni_grid, 'spline');
            % A.Pzeta_bound3 = interp1(lambda_tmp2, Pzeta_tmp2, A.lambda_uni_grid, 'spline');
        ...
        function A = Pzeta_lambda_fixed_mu(qpart,apart,Eperp_in,Pzeta0,Pzeta1,Pzeta_grid_num,lambda_min,lambda_grid_num,sign_gpsi)
            ...
            %tmp = 0.5*(qpart^2/apart)*(A.pzeta_norm*eq.psiw).^2*baxis^2/(gpsi_axis^2) + mu_norm*bmax;%wzk
            ...
            Pzeta_tmp0 = A.pzeta_norm(find(A.pzeta_norm > -1));
            Pzeta_tmp1 = A.pzeta_norm(find(A.pzeta_norm < -1));
            Pzeta_tmp11 = A.pzeta_norm(find(A.pzeta_norm > -1 & A.pzeta_norm < bw));
            Pzeta_tmp12 =[Pzeta_tmp1 Pzeta_tmp11];
            Pzeta_tmp2 = A.pzeta_norm(find(A.pzeta_norm < bw));
            lambda_tmp0 = A.lambda_pzeta(1,find(A.pzeta_norm > -1));
            lambda_tmp1 = A.lambda_pzeta(2,find(A.pzeta_norm < -1));
            lambda_tmp11 = A.lambda_pzeta(4,find(A.pzeta_norm > -1 & A.pzeta_norm < bw));
            lambda_tmp12=[lambda_tmp1 lambda_tmp11];
            lambda_tmp2 = A.lambda_pzeta(3,find(A.pzeta_norm < bw));
