...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function A = cal_char_freq_perturb(np,np_buffer,mstep,tstep,qpart,apart,eq_curr,eq_gradB,E_input,pzeta_input,lambda_input,orb_plot_opt)
    ...
    for istep = 1:mstep
        ...
        for irk = 1:2
            ...
            for i = 1:ant.num_modes
                ...
                phi_r = ant.phi_real{i}(ind_ant);
                phi_i = ant.phi_imag{i}(ind_ant);
                dphi_r = ant.dptdp_real{i}(ind_ant);
                dphi_i = ant.dptdp_imag{i}(ind_ant);
            ...
            for i = 1:ant.num_modes
                phase = - ant.m_modes(i)*tdum + ant.n*zdum - ant.omega*tstep*(istep+0.5*irk);
                apara_r = ant.apara_real{i}(ind_ant);
                apara_i = ant.apara_imag{i}(ind_ant);
                dapara_r = ant.dapdp_real{i}(ind_ant);
                dapara_i = ant.dapdp_imag{i}(ind_ant);
...
if strcmp(ps_option,'PLam_traj')
    ...
    A.E_prime = A.part_pos(:,11,:);
    A.E_power = A.part_pos(:,12,:);
