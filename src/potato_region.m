...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Three curves
xA = PLam_2D.Pzeta(4,:);   yA = PLam_2D.lambda(4,:);
xB = PLam_2D.Pzeta(3,:);   yB = PLam_2D.lambda(3,:);
xC = PLam_2D.Pzeta(4,:);   yC = PLam_2D.lambda(4,:);
...
xC_seg0=xC_seg;
xC_seg = xC_seg(find(xC_seg0>=min(xB_seg)));
yC_seg = yC_seg(find(xC_seg0>=min(xB_seg)));

xC_seg = [xAB xC_seg xB_seg(end)];
yC_seg = [yAB yC_seg yB_seg(end)];

% ---------- Merge three parts into a closed region ----------
x_fill = [xB_seg, xC_seg];
y_fill = [yB_seg, yC_seg];
